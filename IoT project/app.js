...
var websocketclient = {
    ...
    'onConnect': function () {
        ...
        websocketclient.subscribe("temperature", 0, "");
        websocketclient.subscribe("humidity", 0, "");
    },
...
//        console.log("onMessageArrived:" + message.payloadString + " qos: " + message.qos);
    ...
    'render': {
        ...
        'message': function (message) {
            ...
            // Append the latest message to appropriate div based on topic
            if (message.topic === "temperature") {
                $("#temperatureMessages").find("p").text(Encoder.htmlEncode(message.payload));
            } else if (message.topic === "humidity") {
                $("#humidityMessages").find("p").text(Encoder.htmlEncode(message.payload));
            } else {
                // If the topic is neither "temperature" nor "humidity", you can append it to a default div or handle it accordingly
                $("#messEdit").text(Encoder.htmlEncode(message.payload));
            }
